...
error_reporting(0);
$id=$_REQUEST['id'];
$sql1=$conn->query("select * from announcements where id=$id");
...
if(isset($_POST['addannouncements']))
{
    ...
    if($p_image1)
    {
        mysqli_query($conn, "UPDATE `announcements` SET `name` = '$name', `metatitle` = '$metatitle', `image` = '$p_image1', `description` = '$description', `start_date` = '$start_date', `end_date` = '$end_date' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'announcements-list.php';</script>";
    ...
    {
        mysqli_query($conn, "UPDATE `announcements` SET `name` = '$name', `metatitle` = '$metatitle', `description` = '$description', `start_date` = '$start_date', `end_date` = '$end_date' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'announcements-list.php';</script>";
        move_uploaded_file($photo_tmp_name, $target);
        echo mysql_error();
...
<!--                             <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="date" name="end_date" class="form-control" value="<?= $data1['end_date'];?>">
                                </div>
                            </div>     
