...
error_reporting(0);
$id=$_REQUEST['id'];
$sql1=$conn->query("select rules.*, courses.name as course_name from rules left join courses on courses.id = rules.course_id where rules.id=$id");
...
if(isset($_POST['addrules']))
{
    ...
    if($p_image1)
    {
        mysqli_query($conn, "UPDATE `rules` SET `name` = '$name', `metatitle` = '$metatitle', `image` = '$p_image1', `amazon_link` = '$amazon_link', `course_id` = '$course_id', `description` = '$description' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'rules-list.php';</script>";
    ...
    {
        mysqli_query($conn, "UPDATE `rules` SET `name` = '$name', `metatitle` = '$metatitle', `amazon_link` = '$amazon_link', `course_id` = '$course_id', `description` = '$description' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'rules-list.php';</script>";
        move_uploaded_file($photo_tmp_name, $target);
        echo mysql_error();
                    ...
                    <div class="ibox-content">
                        <form method="post" class="form-horizontal" enctype="multipart/form-data" class="parsley-form" id="agent_form">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" name="amazon_link" class="form-control" value="<?= $data1['amazon_link'];?>">
                                </div>
                            </div>      
