...
error_reporting(0);
    if(isset($_GET['verify_token'])){
        ...
        $query = $conn->query("select * from login where verify_token='$verify_token' AND username='$email'");
        $row1 = mysqli_fetch_array($query);
        $token = $row1['verify_token'];
        $username = $row1['username'];
        
        if($token==$verify_token && $username==$email)
        {

            $active = mysqli_query($conn, "update login set `verify_token` = NULL, `status` = 1 where username='$email'");
            if($active)
            {
                echo "<script language='javascript'>alert('Email ID Verification Successfully');window.location = 'login';</script>";;
            }
        }
        else
        {
            echo "<script language='javascript'>alert('Wrong CODE');window.location = 'register';</script>";
        }
    } 
