...
error_reporting(0);
$id=$_REQUEST['id'];
$sql1=$conn->query("select * from downloads where id=$id");
$data1=mysqli_fetch_array($sql1);

if(isset($_POST['adddownloads']))
{
    $structure = "uploads/downloads/";
    ...
    if ($_FILES['p_image1']['type'] != "image/pdf" && $_FILES['p_image1']['type'] != "image/jpeg" 
    && $_FILES['p_image1']['type'] != "image/jpg" && $_FILES['p_image1']['type'] != "image/png") 
    { 
        echo "<script language='javascript'>alert('This file type is not allowed');</script>";
    }
    else 
    {
    ...
    if($p_image1)
    {
        mysqli_query($conn, "UPDATE `downloads` SET `name` = '$name', `metatitle` = '$metatitle', `image` = '$p_image1', `description` = '$description' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'downloads-list.php';</script>";
    ...
    {
        mysqli_query($conn, "UPDATE `downloads` SET `name` = '$name', `metatitle` = '$metatitle', `description` = '$description' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'downloads-list.php';</script>";
        move_uploaded_file($photo_tmp_name, $target);
        echo mysql_error();
...
<!--                             <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="date" name="start_date" class="form-control" value="<?= $data1['start_date'];?>">
                                </div>
                            </div>                                
                            ...
                            <div class="form-group">
                                <div class="col-sm-4 col-sm-offset-2">
                                    ...
                                    <input type="submit" value="Save" name="adddownloads" class="btn btn-primary">
                                </div>
                            </div>
                        </form>
                    </div>
