...
error_reporting(0);
$id=$_REQUEST['id'];
$sql1=$conn->query("select * from admission where id=$id");
$data1=mysqli_fetch_array($sql1);

if(isset($_POST['addadmission']))
{
    $structure = "uploads/admission/";
    ...
    if($p_image1)
    {
        mysqli_query($conn, "UPDATE `admission` SET `name` = '$name', `metatitle` = '$metatitle', `image` = '$p_image1', `description` = '$description' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'admission-list.php';</script>";
    ...
    {
        mysqli_query($conn, "UPDATE `admission` SET `name` = '$name', `metatitle` = '$metatitle', `description` = '$description' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'admission-list.php';</script>";
        move_uploaded_file($photo_tmp_name, $target);
        echo mysql_error();
                        ...
                        <form method="post" class="form-horizontal" enctype="multipart/form-data" class="parsley-form" id="agent_form">
                            ...
                            <div class="form-group">
                                <div class="col-sm-4 col-sm-offset-2">
                                    ...
                                    <input type="submit" value="Save" name="addadmission" class="btn btn-primary">
                                </div>
                            </div>
                        </form>
                    </div>
