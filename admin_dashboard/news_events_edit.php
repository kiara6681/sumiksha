...
error_reporting(0);
$id=$_REQUEST['id'];
$sql1=$conn->query("select * from news_events where id=$id");
$sql1=$conn->query("select news_events.*, courses.name as course_name from news_events left join courses on courses.id = news_events.subject_id where news_events.id=$id");
...
if(isset($_POST['addnews_events']))
{
    ...
    if($p_image1)
    {
        mysqli_query($conn, "UPDATE `news_events` SET `name` = '$name', `metatitle` = '$metatitle', `owner` = '$owner', `image` = '$p_image1', `description` = '$description', `start_date` = '$start_date', `subject_id` = '$subject_id' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'news_events-list.php';</script>";
    }
    
    else
    {
        mysqli_query($conn, "UPDATE `news_events` SET `name` = '$name', `metatitle` = '$metatitle', `owner` = '$owner', `description` = '$description', `start_date` = '$start_date', `subject_id` = '$subject_id' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'news_events-list.php';</script>";
    }
}
...
<!--                             <div class="form-group">
     ...
     <div class="col-sm-10">
         <select name="subject_id" class="form-control" required>
             <option value="<?= $data1['subject_id'];?>"><?= $data1['course_name'];?></option>
         <?php 
         ...
         ?>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" name="owner" class="form-control" value="<?= $data1['owner'];?>">
                                </div>
                            </div>                                
