...
error_reporting(0);
$id=$_REQUEST['id'];
$sql1=$conn->query("select * from courses where id=$id");
...
if(isset($_POST['addcourses']))
{
	...
	if($p_image1)
    {
        mysqli_query($conn, "UPDATE `courses` SET `name` = '$name', `metatitle` = '$metatitle', `image` = '$p_image1', `description` = '$description' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'courses-list.php';</script>";
    ...
    {
        mysqli_query($conn, "UPDATE `courses` SET `name` = '$name', `metatitle` = '$metatitle', `description` = '$description' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'courses-list.php';</script>";
        move_uploaded_file($photo_tmp_name, $target);
        echo mysql_error();
...
<script language="javascript"> 
	...
	$(document).ready(function(e){
		...
		$("#attribute").change(function(){
			...
			$.ajax({
				url : "<?= base_url();?>ajaxPages/getSubAttribute.php",
				type : "POST",
...
<!--                               <div class="form-group">
							...
							<div class="form-group">
                                <div class="col-sm-4 col-sm-offset-2">
                                    ...
                                    <input type="submit" value="Save" name="addcourses" class="btn btn-primary">
                                </div>
                            </div>
                        </form>
                    </div>
