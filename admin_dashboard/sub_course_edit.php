...
error_reporting(0);
$id=$_REQUEST['id'];
$sql1=$conn->query("select sub_course.*, courses.name as course_name , course1.name as sub_course_name from sub_course left join courses on courses.id = sub_course.course_id left join course1 on course1.id = sub_course.sub_course_id where sub_course.id=$id");
...
if(isset($_POST['addcourse1']))
{
    ...
    if($p_image1)
    {
        mysqli_query($conn, "UPDATE `sub_course` SET `name` = '$name', `image` = '$p_image1', `metatitle` = '$metatitle', `description` = '$description', `banner_description` = '$banner_description', `course_id` = '$course_id', `sub_course_id` = '$sub_course_id' WHERE `id` = $id");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'sub_course_list.php';</script>";
    ...
    {
        mysqli_query($conn, "UPDATE `sub_course` SET `name` = '$name', `metatitle` = '$metatitle', `description` = '$description', `banner_description` = '$banner_description', `course_id` = '$course_id', `sub_course_id` = '$sub_course_id' WHERE `id` = $id");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'sub_course_list.php';</script>";
        move_uploaded_file($photo_tmp_name, $target);
        echo mysql_error();
...
<script language="javascript"> 
    ...
    $(document).ready(function(e){
        ...
        var course_id = "<?= $data1['course_id'];?>";
        $('#category option[value='+course_id+']').attr("selected", 'selected');
    });
</script>
                    ...
                    <div class="ibox-content">
                        <form method="post" class="form-horizontal" enctype="multipart/form-data" class="parsley-form" id="agent_form">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <select id="category" name="course_id" class="form-control" required>
                                        <?php 
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <select id="subcategory" name="sub_course_id" class="form-control" required>
                                        <option value="<?= $data1['sub_course_id'];?>"><?= $data1['sub_course_name'];?></option>
                                    </select>
                                </div>
                            </div> 
...
<!--                            <div class="form-group"><label class="col-sm-2 control-label">Banner Description</label>
                                <div class="col-sm-10">
                                    <textarea class="form-control summernote" name="banner_description"><?= $data1['banner_description'];?></textarea>
                                </div>
                            </div> -->
