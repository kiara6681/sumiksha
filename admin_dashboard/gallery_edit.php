...
error_reporting(0);
$id=$_REQUEST['id'];
$sql1=$conn->query("select * from gallery where id=$id");
...
if(isset($_POST['addgallery']))
{
	...
	if($p_image1)
    {
        mysqli_query($conn, "UPDATE `gallery` SET `name` = '$name', `metatitle` = '$metatitle', `image` = '$p_image1', `description` = '$description' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'gallery-list.php';</script>";
    ...
    {
        mysqli_query($conn, "UPDATE `gallery` SET `name` = '$name', `metatitle` = '$metatitle', `description` = '$description' WHERE `id` = $id;");
        echo "<script language='javascript'>alert('Successfully Updated !');window.location = 'gallery-list.php';</script>";
        move_uploaded_file($photo_tmp_name, $target);
        echo mysql_error();
...
<!--                            <div class="form-group">
							...
							<div class="form-group">
                                <div class="col-sm-4 col-sm-offset-2">
                                    ...
                                    <input type="submit" value="Save" name="addgallery" class="btn btn-primary">
                                </div>
                            </div>
                        </form>
                    </div>
